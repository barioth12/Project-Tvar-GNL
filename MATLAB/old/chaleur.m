...
b(1)=u(1)+r/2*(1*u(2)-1*u(1));

b(2:n+1)=u(2:n+1)+r/2*(u(3:n+2)-2*u(2:n+1)+u(1:n));

b(n+2)=r/2*(1*u(n+1)-1*u(n+2))+u(n+2);
...
titre=sprintf('Temps = %0.5g Iteration = %d Pour c effectif et n=%d',t,iteration,n);
title(titre);
