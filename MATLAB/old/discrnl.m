...
for i=1:m;
solip=solip';
k=exp(solip)+exp(-solip);
lh=-tanh(20*(xip/10-1/2))+exp(solip)-exp(-solip);
for j=1:n-2;
    lh(j)=lh(j)-(exp(solip(j))+exp(-solip(j)))*solip(j);
end;
lh=-h*h*lh;
lh(1)=lh(1)+asin(tanh(20*(0/10-1/2))/2)+0.2;
lh(n-1)=lh(n-1)+asin(tanh(20*(10/10-1/2))/2);
diag0=2+h*h*k;
a=diag(diag0)+diag(b,1)+diag(b,-1);
solip=a\(lh)'; 
end;
